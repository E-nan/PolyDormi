...
namespace Poly_Dormitory
{
    public partial class Halla_S_New : Form
    {
        ...
        {
            MySqlCommand cmd1 = new MySqlCommand("INSERT INTO halla_stu values('" + new_Room.Text + "','"       //호실
                                                                                  + new_Name.Text + "','"       //이름
                                                                                  + new_Class.Text + "','"      //학과
                                                                                  + new_Num.Text + "','"        //학번
                                                                                  + grade + "','"               //학년
        ...
        private void Halla_S_New_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand same = new MySqlCommand("SELECT * FROM halla_stat where room = '" + new_Room.Text + "'", mConnection);
            ...
            if (Equals(stuA, ""))
            {
                MySqlCommand cmd2 = new MySqlCommand("UPDATE halla_stat SET stuA='" + new_Name.Text +
            ...
            if (!Equals(stuA, ""))
            {
                MySqlCommand same2 = new MySqlCommand("SELECT * FROM halla_stu WHERE stu_num='" + new_Num.Text + "'", mConnection);
            ...
            if (up == true)
            {
                ...
                {
                    InsertDB();
                    MySqlCommand cmd2 = new MySqlCommand("UPDATE halla_stat SET stuB='" + new_Name.Text +
                                                             "'WHERE room ='" + new_Room.Text + "'", mConnection);
                    cmd2.Connection = mConnection;
                    cmd2.ExecuteNonQuery();
                    up = true;

...
{
    public partial class Halla_Stat : Form
    {
        ...
        private void selectdb()
        {
            ...
            //입.퇴사 현황
            MySqlCommand cmd1 = new MySqlCommand("select * from halla_stu ORDER BY outtime, stu_name", mConnection);
            ...
            //기숙사 현황
            MySqlCommand cmd2 = new MySqlCommand("select * from halla_stat ORDER BY room", mConnection);
            ...
            //시설 현황
            MySqlCommand cmd3 = new MySqlCommand("select * from build_halla ORDER BY ox desc,floor ;", mConnection);
        ...
        private void Halla_Stat_Load(object sender, EventArgs e)
        {
            selectdb();
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            else if (Student_View.SelectedIndices.Count == 1)
            {
                ...
                else
                {
                    ...
                    mConnection.Open();
                    MySqlCommand same1 = new MySqlCommand("SELECT * FROM halla_stat WHERE stuA= '" + name +
                    ...
                    MySqlCommand same2 = new MySqlCommand("SELECT * FROM halla_stat WHERE stuB= '" + name +
                                                                                   "'AND room= '" + room + "'", mConnection);
                    ...
                    if (i != 0)
                    {
                        MySqlCommand up1 = new MySqlCommand("UPDATE halla_stat SET stuA = '" + "" +
                    ...
                    if (j != 0)
                    {
                        MySqlCommand up2 = new MySqlCommand("UPDATE halla_stat SET stuB = '" + "" +
                                                                             "'WHERE room='" + room + "'", mConnection);
                        up2.Connection = mConnection;
                        up2.ExecuteNonQuery();
                    }

                    MySqlCommand cmd = new MySqlCommand("DELETE FROM halla_stu WHERE stu_num = '" + num + "'", mConnection);
                    cmd.Connection = mConnection;
                    cmd.ExecuteNonQuery();

                    //학생을 삭제하면 현재 현황에도 업데이트
                    Now_View.Items.Clear();
                    MySqlCommand cmd2 = new MySqlCommand("select * from halla_stat;", mConnection);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            else if (Build_View.SelectedIndices.Count == 1)
            {
                ...
                {
                    mConnection.Open();
                    MySqlCommand cmd = new MySqlCommand("DELETE FROM build_halla WHERE build_name= '" + info + "'", mConnection);
                    cmd.Connection = mConnection;
                    cmd.ExecuteNonQuery();
                    mConnection.Close();
        ...
        {
            if (MessageBox.Show("전체 데이터가 삭제됩니다. 계속 하시겠습니까?", "경고", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
            {// no 일때 
            ...
            {//yes 일때
                ...
                {
                    mConnection.Open();
                    MySqlCommand all1 = new MySqlCommand("DELETE FROM halla_stu ;", mConnection);
                    all1.Connection = mConnection;
                    all1.ExecuteNonQuery();

                    MySqlCommand all2 = new MySqlCommand("UPDATE halla_stat set stuA='" + "" +
                                                                             "',stuB='" + "" + "'", mConnection);
                    all2.Connection = mConnection;
                    all2.ExecuteNonQuery();

                    MySqlCommand all3 = new MySqlCommand("DELETE FROM build_halla ;", mConnection);
